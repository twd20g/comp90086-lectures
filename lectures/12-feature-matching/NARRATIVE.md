...
| **Slide 27's placeholder became `components/homography-live.html`** | Detect, describe, match, then vote over 4-point samples — the whole lecture, on two frames the room supplies. Ends with the first frame warped onto the second, because a bad fit shows as ghosting and no number on a slide can be read the way a doubled edge can, and then with the optional refit as its own step: the sample's model is kept rather than overwritten so both quads can be drawn together and the correction stated as a number. |
